...
public class RoomTest02_1 {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		
//0		int[] room = new int[12];            // ?˜ˆ?•½?ƒ?ƒœ.
//1		int[] roomNo = new int[12];          // ?˜¸?‹¤ë²ˆí˜¸
//2		int[] code = new int[12];            // ?‚¬?š©?ê°? ?…? ¥?•˜?Š” ?˜ˆ?•½ë²ˆí˜¸ 
		
		final  int AROOM     = 0;
		final  int AROOMNO = 1;
		final  int ACODE     = 2;
		int[][] room = new int[3][12];            // ?˜ˆ?•½?ƒ?ƒœ.
			...
			switch(nSelectMenu) {
						...
						while (!(0 <= nReserve)&&(nReserve <= 11)) {  // ? •?ƒ? ?œ¼ë¡? ?˜ˆ?•½ ë²ˆí˜¸ë¥? ë°›ìœ¼ë©? ?‹œ???Š¤ê°? ??‚¨.
							...
							if ((0 <= nReserve)&&(nReserve <= 11)) {  // ? œ??ë¡? ?…? ¥?–‡?œ¼ë©?
								if (room[AROOM][nReserve] == 0) {
									...
									System.out.printf("%2d ?˜¸?‹¤(%2d): ?˜ˆ?•½ê°??Š¥ %n",room[AROOMNO][nReserve],nReserve);
									...
									while(nCode == -1) {  
										...
										for (i=0;i<room[ACODE].length;i++) {
											if (room[ACODE][i] == nCode) {
												bCheck = true;
												nCode  = -1;
												System.out.println("?´ë¯? ?‚¬?š©ì¤‘ì¸ ?˜ˆ?•½ë²ˆí˜¸?…?‹ˆ?‹¤.");
												break;
											}
										}
										if (nCode != -1) {
									...
									System.out.printf("%dë²? ?˜¸?‹¤(%2d) %s ?‹˜ : ?˜ˆ?•½?¨:?˜… ?˜ˆ?•½ë²ˆí˜¸:%d%n%n",room[AROOMNO][nReserve],nReserve, User[nReserve], room[ACODE][nReserve]);
									room[AROOM][nReserve] = 1;
								} else {                               
									System.out.printf("%dë²? ?˜¸?‹¤(%2d)?? ?´ë¯? ?˜ˆ?•½?œ ë°©ì…?‹ˆ?‹¤, ?˜ˆ?•½?´ ë¶ˆê??Š¥ ?•©?‹ˆ?‹¤.%n%n",room[AROOMNO][nReserve],nReserve);
									nReserve = -1;
								}
					...
					while (!(0 <= nReserve)&&(nReserve <= 11)) {     // ? •?ƒ? ?œ¼ë¡? ?˜ˆ?•½ì·¨ì†Œê°? ??‚˜ë©? ?‹œ???Š¤ ì¢…ë£Œ.
						...
						if ((0 <= nReserve)&&(nReserve <= 11)) {     // ? œ??ë¡? ?…? ¥?–ˆ?œ¼ë©?
							if (room[AROOM][nReserve] == 1) {
								System.out.print("?˜ˆ?•½?„ ?™•?¸ ?•©?‹ˆ?‹¤, ë©”ë‰´ë¥? ?„ ?ƒ?•´ì£¼ì„¸?š”. \n1:?„±?•¨?™•?¸ , 2:?˜ˆ?•½ë²ˆí˜¸?™•?¸ :");
								...
								switch(nSubMenu) {
									case 1 : // ?„±?•¨
										...
										if (sUserName.equals(User[nReserve])) {
											System.out.printf("%dë²? ?˜¸?‹¤(%2d) : (?˜ˆ?•½ë²ˆí˜¸:%d) %s?‹˜ ?˜ˆ?•½ì·¨ì†Œ ?™„ë£? %n%n",room[AROOMNO][nReserve] ,nReserve,room[ACODE][nReserve],  User[nReserve] );
				...
				case 3  :  //==================================?˜¸?‹¤ë³´ê¸° 3==========================//
					...
					for (i=0;i<room[AROOM].length;i++) {
						str = "";
						if (room[AROOM][i] == 0) {
							str = String.format("%3dë²? ?˜¸?‹¤(%2d) : ?˜ˆ?•½ê°??Š¥?˜† " , room[AROOMNO][i],  i );
						} else if(room[AROOM][i] == 1) {
							str = String.format("%3dë²? ?˜¸?‹¤(%2d) : ?˜ˆ?•½?¨:?˜… ?˜ˆ?•½? :%5s ?˜ˆ?•½ë²ˆí˜¸ :%5d",  room[AROOMNO][i],  i, User[i], room[ACODE][i]);
						} else {
	...
	//============ ?‚¬?š©?ê°? ?…? ¥?•œ ë£¸ì´ë¦? ?„ ë°°ì—´?—?„œ ì°¾ì•„?„œ ?¸?±?Š¤ë¥? ë¦¬í„´?•œ?‹¤.
