...
public class VendingM_v2_update {
	...
	public static void main(String[] args) {
		...
		String[] sMenu = {"프라푸치노", "제로펩시","나랑드사이다",
				         "자몽에이드", "98.종료"  ,"99.잔액반출"};
...
//		거스름돈 배열구성하기 - 만원 10개, 오천원 7개 천원 오백원 백원
		...
		while (nSelectmenu != 98) {
			...
			for(i=0;i < nMenu[PRICE].length;i++) {
				...
				str += String.format("%s",((ntmp > 0) ?"판매중":"품절") );
				System.out.println(str);
				//System.out.printf("%d.%s(%d원)    %s%n",, , nMenu[PRICE][i], ((ntmp > 0) ?"판매중":"품절") );
			} 
				...
				case 1 : case 2 : case 3 : case 4 :
					...
					if (nMoney >= npricevalue) {
						if (nstockvalue > 0) {
							...
							System.out.printf("%s 가 나왔습니다. 남은잔액: %d원%n", sMenuvalue, nMoney);
						} else {
...
//					메소드 구성하는 게 편함.
	...
	static int Returnofchange(int value ) {
		if (value > 0) {
			if (value >= 10000) {
				System.out.printf("10000원 %d장,", (value/10000));
				value = value % 10000;
			} 
			if (value >= 1000) {
