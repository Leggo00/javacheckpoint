...
public class VendingM_v3_1_error {
	...
	private static int VendingRun(int aMoney, String[][] sMenu, int[] changeM, String sPassword) {
		...
		switch (nSelectmenu) {
			...
			case 100 :
				...
				int error = AdiminRUN(sMenu);           // 음료변경및 재고
				printError(error);
				return aMoney;
			default: 
				return aMoney;
...
//	프로그램 상의 에러코드
	private static void printError(int error) {
		...
		switch(error) {
		case 199 : case 201 :case 203 :case 205 :
			System.out.println(error+" :메뉴변경을 중단하고 초기메뉴로 돌아갑니다.");
			break;
		case 200 :
			System.out.println(error+" :음료 메뉴 번호가 정상적으로 들어오지 않았습니다. (1~7)사이의 수를 입력해주세요.");
			break;
		case 202 :
			System.out.println(error+" :음료의 이름이 정상적으로 들어오지 않았습니다.");
			break;
		case 206 :
			System.out.println(error+" :변경한 음료의 재고값이 정상적으로 들어오지 않았습니다.");
			break;
		}
	}


	private static int AdiminRUN(String[][] sMenu) {
